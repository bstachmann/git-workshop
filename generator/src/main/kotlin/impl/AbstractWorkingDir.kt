...
abstract class AbstractWorkingDir<T>(
        ...
        fun createFile(name: String, content: String? = null, commands: File.() -> Unit = {}) =
                File(java.io.File(rootDir, name).absoluteFile, log, solutionCollector)
                        .apply {
                                if (location.exists())
                                        throw IllegalStateException(
                                                "File $location is not expected to exist!"
                                        )
                        }
        ...
        fun inFile(name: String = "file", commands: File.() -> Unit = {}) =
                File(java.io.File(rootDir, name), log, solutionCollector)
                        .apply {
                                if (!location.exists())
                                        throw IllegalStateException(
                                                "File $location is expected to exist!"
                                        )
                        }
                        .run(commands)

        fun git(commands: GitContext<T>.() -> Unit = {}) = GitContext<T>(this).commands()
        ...
        fun ll(additionalArgs: String = "") =
                bash(
                        "ls -lh --time-style=+\'\' $additionalArgs",
                        commandRepresentation = "ll $additionalArgs"
                )

        fun bash(
                command: String,
                acceptableExitCodes: Set<Int> = setOf(0),
                commandRepresentation: String = command
        ): List<String> {
                val process =
                        executeProcess(
                                "/bin/bash",
                                ...
                                stdoutRedirect = PIPE,
                                errorRedirect = PIPE,
                                validateOutcome = { p: Process ->
                                        assertExitCode(p, acceptableExitCodes, command)
                                }
                        )
        ...
        fun executeProcess(
                ...
                validateOutcome: (Process) -> Unit = { p: Process ->
                        assertExitCode(p, setOf(0), arguments)
                }
        ): Process {
                ...
                processBuilder.environment()["GIT_AUTHOR_EMAIL"] =
                        userEmail ?: "kapitel26blog@gmail.com"
                processBuilder.environment()["GIT_COMMITTER_NAME"] = userName ?: "bjoern"
                processBuilder.environment()["GIT_COMMITTER_EMAIL"] =
                        userEmail ?: "kapitel26blog@gmail.com"
        ...
        fun getLocalGitConfig(propertyName: String): String? {
                val processBuilder =
                        ProcessBuilder(listOf("git", "config", "--local", propertyName))
                processBuilder.directory(rootDir)
        ...
        private fun assertExitCode(
                p: Process,
                expectedExits: Set<Int>,
                splittedCommandLineArguments: Array<out String>
        ) {
                if (p.exitValue() !in expectedExits)
                        throw CommandLineException(
                                p,
        ...
        fun createRepo(
                newRepoName: String = "repo",
                vararg args: String,
                commands: (Repo.() -> Unit)? = null
        ) {
                git("init -b main $newRepoName ${args.joinToString(" ")}")
                Repo(
                        java.io.File(rootDir, newRepoName).absoluteFile,
                        log,
                        solutionCollector,
                        commands
                )
        ...
        fun inRepo(repoName: String = "repo", commands: (Repo.() -> Unit)? = null): Unit =
                java.io.File(rootDir, repoName)
                        .absoluteFile
                        .apply {
                                if (!exists())
                                        throw IllegalStateException("Repo $this does not exist!")
                        }
                        .run { if (commands != null) Repo(this, log, solutionCollector, commands) }
        ...
        fun createIntro(title: String, description: String = "", setup: T.() -> Unit = {}) {
                ...
                val header = ("Schritt" en "Step") + " 0 - START"
                ...
                markdown(
                        "<h2>${header} <!-- UEB/${solutionCollector.aufgabenName()}/${aufgabenNr} --></h2>"
                )
        }
        ...
        fun createAufgabe(
                ...
                val header =
                        ("Schritt" en "Step") +
                                " ${solutionCollector.collectedCommands.size} - $title"
                val aufgabenNr = solutionCollector.collectedCommands.size
                solutionCollector.registerSchritt(header)
                ...
                markdown(
                        ("Starte im Verzeichnis" en "Start in directory") +
                                " `${startPath ?: pathInUebungsverzeichnis}`."
                )
                markdown(description)
        }
        ...
        fun editAndCommit(
                ...
                line: Int,
                message: String =
                        "Edit file $fileName at line $line on branch ${currentBranch()} by ${currentUser()}."
        ...
        fun editAndCommit(
                ...
                lines: IntRange,
                message: String =
                        "Edit file $fileName at lines $lines on branch ${currentBranch()} by ${currentUser()}."
        ) {
        ...
        fun editAndCommit(
                fileName: String,
                message: String =
                        "Edited file $fileName on branch ${currentBranch()} by ${currentUser()}.",
                commands: File.() -> Unit = {}
        ) {
        ...
        fun edit(fileName: String, commands: File.() -> Unit = {}) {
                inFile(fileName, commands)
        }
        ...
        fun createFileAndCommit(
                fileName: String,
                message: String =
                        "Created file $fileName on branch ${currentBranch()} by ${currentUser()}.",
                commands: File.() -> Unit = {}
        ) {
                createFile(fileName)
                inFile(fileName, commands)
        ...
        fun currentBranch(): String {
                val lines =
                        executeProcess("git", "symbolic-ref", "--short", "HEAD")
                                .inputStream
                                .bufferedReader()
                                .readLines()
